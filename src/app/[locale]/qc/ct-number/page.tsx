...
export default function CtNumberPage() {
	...
	const result = useMemo(() => {
		...
		const worstUniformity = peripheral.reduce(
			(worst, p) =>
				Math.abs(p - center) > Math.abs(worst) ? p - center : worst,
			0,
		);
			...
			<main className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-6 py-10 sm:px-8">
				<header className="flex flex-col gap-2">
					...
					<p className="text-sm text-zinc-600 dark:text-zinc-400">
						...
						(top/right/bottom/left). Reports CT number accuracy of water (target
						0 HU), worst uniformity deviation, and image noise (ROI standard
						deviation).
					</p>
					<div className="pt-1">
						<EquipmentSelector calculatorSlug="qc/ct-number" />
					</div>
				</header>
				...
				<footer className="border-t border-radiation-400/20 pt-4 text-xs text-zinc-400">
					...
					<p>
						Uniformity: largest |peripheral − centre| across the four peripheral
						ROIs.
					</p>
					<p>
						Noise: ROI standard deviation. Cap is protocol-specific; displayed
						cap is a typical 7 HU at standard adult-abdomen technique.
					</p>
				</footer>
			</main>
		</div>
